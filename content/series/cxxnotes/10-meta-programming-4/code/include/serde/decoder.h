...
extern std::unordered_map<std::uintptr_t,
...
struct DataDecoder<T, std::enable_if_t<reflect::is_ast_node_v<T>>> {
  void operator()(std::istream &in_stream, T &object) {
    ...
    if constexpr (Access::kHasSuper)
      load_as<typename Access::super_type>(object);
    load_fields(in_stream, object,
                std::make_index_sequence<Access::kNumFields>{});
  }
 ...
 private:
  ...
  bool load_fields(std::istream &in_stream, T &object,
...
struct DataDecoder<T *> {
  void operator()(T *&ptr, std::istream &in_stream) {
    ...
    if constexpr (reflect::is_ast_node_v<T>) {
      rfe_old_addr_to_rfr[reinterpret_cast<std::uintptr_t>(ptr)].emplace_back(
          &ptr);
    }
  }
};
